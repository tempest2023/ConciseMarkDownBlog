...
Answers were initially too expansive. The prompt now targets 80–120 English words or 150–220 Chinese characters, representative examples instead of exhaustive biographies, unchanged English personal names, and one or two source links even in follow-ups. These are model instructions, not hard output-format guarantees. Live checks also encountered an upstream free-tier 429 limit. Browser verification confirmed the specific busy/retry message, exit from loading state, preserved question, and working New conversation action. No automatic retries, billing changes, or quota workarounds were used.
