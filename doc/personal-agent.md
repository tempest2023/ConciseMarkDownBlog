...
3. For local preview, create a gitignored `.env.local` using `.env.example` as the template. For deployment, add the same server-only variables in the target Vercel project's environment settings: `AI_GATEWAY_API_KEY`, `PERSONAL_AGENT_ENABLED=true`, and optionally `AGENT_MODEL`. It accepts an ordered comma-separated list: the first entry is the primary model and later entries are AI Gateway fallbacks. The default is `inception/mercury-2.5,alibaba/qwen3.8-flash`. Never use a `REACT_APP_` prefix and never commit credentials.
...
A key and a spend budget alone do not prove the account is ready to serve every model. If requests fail, review the Gateway dashboard for account or model eligibility. Keep payment information out of the repository and chat. The public chat never exposes raw provider/account errors. Upstream 429 responses show a wait-and-retry message; user retries remain manual. AI Gateway tries the configured backup models in order when the primary model fails. The server streams the successful Gateway model back to the browser, and the composer updates to the model that actually served the reply. Waiting a minute is a suggested first step, not a promise about the provider's quota-reset time.
...
Each reply uses no same-model retries, at most 600 output tokens, a 25-second timeout, and at most seven messages / 8,000 conversation characters. Reasoning is disabled because this public guide needs short direct answers; this also prevents reasoning-only output from exhausting Mercury's output limit before visible text is produced. If the primary model fails, AI Gateway may try the configured backup. Each message is limited to 2,000 characters; the HTTP body to 18,000 bytes. Only the last three complete turns are retained as input, trimmed further if needed. The fixed public context is resent each time; the context-size regression test flags growth above 40,000 characters. Character limits are not token counts or a precise price estimate.
...
The application does not persist conversations on the server or log prompts, replies or raw provider errors. Complete conversation pairs are saved in the visitor's browser using `localStorage`, so they load automatically after a refresh or when the visitor reopens the agent on that browser. The collapsible chat sidebar manages up to 20 recent conversations, each retaining the last 20 complete turns. + New chat keeps earlier conversations and opens an empty chat; sidebar deletion removes a chosen conversation after confirmation. The selected conversation is restored on reload. Existing single-conversation storage is migrated only after writing the replacement successfully. Streaming does not write history on each delta, and changing conversations cancels the outgoing request and ignores its late events. Messages still pass through Vercel and the model provider, whose retention settings and policies apply. In-memory throttling uses a daily hashed client address. A visitor should not submit private information. Stopping a reply aborts the upstream request but does not erase already incurred usage.
