...
function loadComponent(relativePath) {
  ...
  const source = transformSync(fs.readFileSync(filename, 'utf8'), { configFile: false, babelrc: false, presets: ['@babel/preset-react'], plugins: ['@babel/plugin-transform-modules-commonjs'] }).code;
  const loaded = { exports: {} };
  new Function('module', 'exports', 'require', source)(loaded, loaded.exports, createRequire(filename));
  return loaded.exports.default;
}
const config = loadComponent('src/config.js');
...
function documentFor(entry, markdown, { noindex = false, utility = false } = {}) {
  ...
  return `<!doctype html><html lang="${seo.language}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${escape(title)}</title><meta name="description" content="${escape(description)}"><meta name="author" content="${escape(seo.author)}"><meta name="robots" content="${noindex ? 'noindex, follow' : 'index, follow, max-image-preview:large'}"><link rel="canonical" href="${escape(canonical)}"><link rel="icon" type="image/png" sizes="256x256" href="/assets/agent-avatar/idle.png"><link rel="alternate" type="application/rss+xml" title="RSS" href="/rss.xml"><meta property="og:type" content="${entry.isPost ? 'article' : 'website'}"><meta property="og:title" content="${escape(title)}"><meta property="og:description" content="${escape(description)}"><meta property="og:url" content="${escape(canonical)}"><meta property="og:image" content="${escape(new URL(seo.defaultImage, base).href)}"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${escape(title)}"><meta name="twitter:description" content="${escape(description)}"><meta name="twitter:image" content="${escape(new URL(seo.defaultImage, base).href)}"><script id="seo-structured-data" type="application/ld+json">${json(noindex ? {} : structured)}</script>${assets}<noscript><style>.site-nav{display:flex;position:static;flex-wrap:wrap}.site-header{flex-wrap:wrap}.header-controls{flex-wrap:wrap}</style></noscript></head><body><div id="root"><div class="page"><a class="skip-link" href="#main-content">Skip to content</a><header class="site-header"><a class="site-brand" href="/">${escape(config.title)}</a><nav class="site-nav" aria-label="Main navigation">${nav.map(item => `<a href="${escape(item.href)}"${entry.path === item.href ? ' aria-current="page"' : ''}>${escape(item.label)}</a>`).join('')}</nav></header><div class="notebook-layout">${sidebar}<main id="main-content" class="main-container"><div class="container"><div class="article-shell"><article class="article-content">${dates}${!hasTitle ? `<h1>${escape(entry.title)}</h1>` : ''}${body}</article></div></div></main></div><footer class="static-footer"><a href="mailto:${escape(config.email)}">Email</a><a href="${escape(config.resume_url)}">Résumé</a><a href="${escape(config.social.github)}">GitHub</a><a href="${escape(config.social.linkedin)}">LinkedIn</a><div class="static-colophon"><p>A small space for unfinished thoughts and things worth sharing.</p><span>— Tempest</span></div><nav class="static-footer-links" aria-label="More links"><a href="${escape(config.repo)}">Made of Markdown ↗</a><a href="/rss.xml">RSS</a></nav><p>© ${new Date().getFullYear()} ${escape(config.name)}</p></footer></div></div><script id="blog-bootstrap" type="application/json">${json({ page: entry.page, markdown: utility ? '' : markdown, noIndex: noindex })}</script></body></html>`;
}
